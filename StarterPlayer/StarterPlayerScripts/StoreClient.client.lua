...
local tabsFrame=Instance.new("Frame");tabsFrame.BackgroundTransparency=1;tabsFrame.Size=UDim2.new(0,180,1,-110);tabsFrame.Position=UDim2.new(0,18,0,104);tabsFrame.Parent=panel
...
local upgrades={
    {"Timing","Timing","Makes Good hits slightly easier.",120,"Recommended"},
    {"HypeGain","Hype Gain","Build Hype faster from clean hits.",150,"Recommended"},
    {"Recovery","Recovery","Miss streaks hurt less.",150,""},
    {"Stagecraft","Stagecraft","Better effects and more Fans.",170,""},
    {"Focus","Focus","Resist battle distractions.",180,""},
    {"CoinBonus","Coin Bonus","Earn more Coins per song.",160,"Recommended"},
    {"AudiencePower","Audience Power","Audience support gives more Hype.",160,""},
}

local function owned(category,id)
    return snapshot.OwnedCosmetics and snapshot.OwnedCosmetics[category] and snapshot.OwnedCosmetics[category][id]
end
local function equipped(category,id)
    return snapshot.Equipped and snapshot.Equipped[category]==id
end
local function canAfford(currency,cost)
    return cost<=0 or ((snapshot[currency] or 0) >= cost)
end
...
local function card(y,titleText,desc,cost,currency,badge,onBuy,onEquip,buyText,equipText,affordable)
    local c=Instance.new("Frame");c.Size=UDim2.new(1,-12,0,118);c.Position=UDim2.new(0,0,0,y);c.BackgroundColor3=Color3.fromRGB(24,28,48);c.Parent=list;corner(c,14);stroke(c,Color3.fromRGB(80,225,255))
    ...
    label(c,desc,UDim2.new(.62,0,0,60),UDim2.new(0,14,0,46),Color3.fromRGB(215,225,255),Enum.Font.GothamBold)
    label(c,cost.." "..currency,UDim2.new(.25,0,0,30),UDim2.new(.67,0,0,12),affordable~=false and Color3.fromRGB(255,240,160) or Color3.fromRGB(255,120,120),Enum.Font.GothamBlack)
    local buy=button(c,buyText or "Buy",UDim2.new(0,112,0,38),UDim2.new(1,-248,1,-50),affordable~=false and Color3.fromRGB(55,145,255) or Color3.fromRGB(95,95,110));buy.Activated:Connect(onBuy)
    if onEquip then local eq=button(c,equipText or "Equip",UDim2.new(0,112,0,38),UDim2.new(1,-126,1,-50),Color3.fromRGB(120,200,95));eq.Activated:Connect(onEquip) end
end
...
local function render()
    ...
    elseif currentTab=="Upgrades" then
        for i,u in ipairs(upgrades) do
            ...
            card((i-1)*128,name,"Level "..lvl.." / 5\n"..desc,finalCost,"Coins",tag,function() remotes.PurchaseItem:FireServer({category="GameplayUpgrades",itemId=id}) end,nil,lvl>=5 and "Maxed" or "Buy Upgrade",nil,canAfford("Coins",finalCost))
        end
        list.CanvasSize=UDim2.new(0,0,0,#upgrades*128+20)
    elseif currentTab=="Missions" then
        local missions={{"Get 50 Perfects","0 / 50","Reward: Fans + Coins + XP"},{"Finish 3 songs","0 / 3","Reward: Tickets + XP"},{"Reach 80 Hype","0 / 1","Reward: Fans"},{"Cheer for 2 performances","0 / 2","Reward: Audience XP"}}
        for i,m in ipairs(missions) do card((i-1)*128,m[1],m[2].."\n"..m[3],0,"", "Daily", function() end, nil, "Server tracks", nil, true) end
        list.CanvasSize=UDim2.new(0,0,0,#missions*128+20)
    elseif currentTab=="Tour Bus" then
        local bus={{"Bigger Speakers","+Hype gain"},{"Snack Stand","Passive Fans placeholder"},{"Practice Seat","+XP from retries"},{"Merch Box","+Fans from audience"},{"Road Crew","Lowers venue fees"},{"Neon Wrap","Cosmetic hub style"}}
        for i,b in ipairs(bus) do card((i-1)*128,b[1],b[2].."\nComing as progression hub upgrades.",0,"", "Hub", function() end, nil, "Coming Soon", nil, true) end
        list.CanvasSize=UDim2.new(0,0,0,#bus*128+20)
    else
