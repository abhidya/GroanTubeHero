...
The generator uses a small hardcoded deterministic lane key (`GROAN_TUBE_EASY_KEY_V1`) so the same audio/timestamps produce stable arrow-lane assignments.
...
Only rights-cleared local/test charts should be visible in-game; there is no paid power unlock path.
