...
## 2026-05-25 Creator Store MCP menu/world expansion import batch

This batch used Roblox Studio MCP Creator Store search/insert into `Workspace.AssetInbox`, quarantined imported scripts into `ServerStorage.AssetQuarantine`, promoted clean visual-only models into `ReplicatedStorage.ArtAssets`, then placed cloned active art under `Workspace.GTH_WorldV2`. This proves 12 Creator Store source asset IDs and 1,180 unique named active placements from those audited sources; it does **not** prove 1,000 distinct source asset IDs.

| Source asset ID/path | Query / imported root | Cleaned ArtAssets path | Used WorldV2 path | Scripts quarantined | Purpose |
| --- | --- | --- | --- | ---: | --- |
| Creator Store `86055952988764` | `concert stage` / `Workspace.AssetInbox.CS_ConcertStage` | `ReplicatedStorage.ArtAssets.Stage.Clean_Creator_CS_ConcertStage` | Promoted source; not used in 1,180 lightweight placement pass because source is very large | included in batch total | Stage source candidate |
| Creator Store `5564827474` | `stage truss` / `Workspace.AssetInbox.CS_StageTruss` | `ReplicatedStorage.ArtAssets.Stage.Clean_Creator_CS_StageTruss` | `StageCircle.CreatorMenuExpansion_stageCore`, `LightingAnchors.CreatorMenuExpansion_lightingAndTrusses`, `TourBusAndSpawnDressing.CreatorMenuExpansion_tourBusAndSpawn` | included in batch total | Stage/truss/spawn dressing |
| Creator Store `85589657655640` | `concert lights` / `Workspace.AssetInbox.CS_ConcertLights` | `ReplicatedStorage.ArtAssets.Lighting.Clean_Creator_CS_ConcertLights` | `LightingAnchors.CreatorMenuExpansion_lightingAndTrusses` | included in batch total | Concert lighting |
| Creator Store `5243789408` | `speakers` / `Workspace.AssetInbox.CS_Speakers` | `ReplicatedStorage.ArtAssets.Stage.Clean_Creator_CS_Speakers` | Promoted source; not used in 1,180 lightweight placement pass | included in batch total | Speaker source candidate |
| Creator Store `9631673829` | `DJ booth` / `Workspace.AssetInbox.CS_DJBooth` | `ReplicatedStorage.ArtAssets.Stage.Clean_Creator_CS_DJBooth` | Promoted source; not used in 1,180 lightweight placement pass | included in batch total | DJ booth source candidate |
| Creator Store `13086664189` | `cash register` / `Workspace.AssetInbox.CS_CashRegister` | `ReplicatedStorage.ArtAssets.Props.Clean_Creator_CS_CashRegister` | `VendorRing.CreatorMenuExpansion_vendorRing` | included in batch total | Store/vendor props |
| Creator Store `144012624` | `security console` / `Workspace.AssetInbox.CS_SecurityConsole` | `ReplicatedStorage.ArtAssets.Props.Clean_Creator_CS_SecurityConsole` | `VendorRing.CreatorMenuExpansion_vendorRing`, `FenceRing.CreatorMenuExpansion_fenceRing` | included in batch total | Security/fence/vendor console props |
| Creator Store `3215211735` | `tour bus` / `Workspace.AssetInbox.CS_TourBus` | `ReplicatedStorage.ArtAssets.TourBus.Clean_Creator_CS_TourBus` | Promoted source; not used in 1,180 lightweight placement pass | included in batch total | Tour bus source candidate |
| Creator Store `13462776766` | `monster crowd` / `Workspace.AssetInbox.CS_MonsterCrowd` | `ReplicatedStorage.ArtAssets.Horde.Clean_Creator_CS_MonsterCrowd` | Promoted source; source inspection found no countable model parts in the lightweight placement pass | included in batch total | Horde source candidate |
| Creator Store `8220780721` | `cartoon NPC` / `Workspace.AssetInbox.CS_CartoonNPC` | `ReplicatedStorage.ArtAssets.Vendors.Clean_Creator_CS_CartoonNPC` | `AudienceRing.CreatorMenuExpansion_audienceRing` | included in batch total | Audience/NPC dressing |
| Creator Store `907464004` | `lava rock` / `Workspace.AssetInbox.CS_LavaRock` | `ReplicatedStorage.ArtAssets.Volcano.Clean_Creator_CS_LavaRock` | `HordeRing.CreatorMenuExpansion_hordeRing`, `VolcanoOuterRing.CreatorMenuExpansion_volcanoOuterRing` | included in batch total | Horde/volcano rock dressing |
